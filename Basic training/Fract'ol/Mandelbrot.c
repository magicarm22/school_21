...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/20 17:01:52 by djast             #+#    #+#             */
/*   Updated: 2019/03/20 17:02:10 by djast            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

t_julia *init_mandelbrot(t_mlx *mlx)
{
	t_julia *julia;
...
void mandelbrot_calculate(t_complex *clx, t_mlx *mlx, int x, int y, t_julia *julia)
{
	int iteration;

	iteration = julia->max_step;
	julia->const_C->c = 1.5 * (x - SIZE_MAP_X / 2)/(0.5 * mlx->zoom *  SIZE_MAP_X) + mlx->place_x - 0.58;
	julia->const_C->i = (y - SIZE_MAP_Y / 2) / (0.5 * mlx->zoom * SIZE_MAP_Y) + mlx->place_y;
	clx->c = 0;
	clx->i = 0;
	while (clx->c*clx->c + clx->i*clx->i < 4 && --iteration > 0)
	{
		julia->tmp = clx->c;
		clx->c = clx->c * clx->c - clx->i * clx->i + julia->const_C->c;
		clx->i = 2.0 * julia->tmp * clx->i + julia->const_C->i;
	}
...
void *mandelbrot_fractol(void *arguments)
{
	t_julia *julia;
	t_thread *args;
	...
	julia = init_mandelbrot(args->mlx);
	...
	while (args->cur_x < args->max_x)
	{
		...
		{
			mandelbrot_calculate(args->clx, args->mlx, args->cur_x, args->cur_y, julia);
        	args->cur_y++;
		}
		args->cur_x++;
	}
...
void mandelbrot_threads(t_complex *clx, t_mlx *mlx)
{
	...
	while (i < NUM_THREADS)
	{
		...
		args[i]->num_thread = i;
		pthread_create(&threads[i], NULL, mandelbrot_fractol, (void *)args[i]);
		i++;
	}
	while (i-- >= 0)
